"""
PowerTeamit Telegram Bot - Compatible with python-telegram-bot 21.x
"""
...
async def get_main_image(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ تصویر اصلی ثبت شد.\n\n📸 اگه عکس اضافه داری بفرست، وگرنه دکمه رو بزن:",
...
async def get_extra_images(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("⚠️ لطفاً عکس بفرست یا دکمه رو بزن.")
    return EXTRA_IMAGES


async def skip_extra_images(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.message.reply_text("✅ عکس‌ها ثبت شدن.\n\n💰 قیمت پیشنهادی (دلار):\nمثال: 25.00")
    return PRICE
...
async def submit_listing(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.callback_query.message.reply_text(f"⚠️ خطا: {e}")
