...
ربات دریافت موجودی از فروشنده‌ها
...
# ─── تنظیمات ───────────────────────────────────────────────
BOT_TOKEN = "8885657885:AAELo212q5imxrrH17P6CMNdj2wIdOzgUtI"          # توکن ربات از @BotFather
ADMIN_CHAT_ID = 1341460786                  # آیدی عددی تلگرام خودت
...
async def get_game_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ ثبت شد.\n\n"
        "📝 حالا یه تایتل جذاب برای اکانت بنویس:\n"
...
async def get_description(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ ثبت شد.\n\n"
        "🖼 حالا تصویر اصلی (main image) اکانت رو بفرست:\n"
        "این تصویر به عنوان کاور نمایش داده میشه."
    )
    return MAIN_IMAGE
...
async def get_main_image(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("✅ بدون عکس اضافه، ادامه بده", callback_data="skip_extra")]
    ]
    await update.message.reply_text(
        "✅ تصویر اصلی ثبت شد.\n\n"
        "📸 اگه عکس‌های بیشتری داری، همشون رو یکی‌یکی بفرست.\n"
...
async def get_extra_images(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        keyboard = [
            [InlineKeyboardButton(f"✅ همین {count} تا کافیه، ادامه بده", callback_data="skip_extra")]
        ]
        await update.message.reply_text(
            f"📸 عکس {count} ثبت شد. می‌تونی بیشتر بفرستی یا ادامه بدی:",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return EXTRA_IMAGES
    else:
...
async def get_price(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "(یعنی اطلاعات ورود رو همون لحظه تحویل میدی؟)",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    return IS_INSTANT


# ─── مرحله ۷: instant یا نه ────────────────────────────────
...
async def submit_listing(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ارسال تصویر اصلی + پیام به ادمین
    try:
        ...
        if extra:
            ...
            await ctx.bot.send_media_group(chat_id=ADMIN_CHAT_ID, media=media)

    except Exception as e:
        logger.error(f"خطا در ارسال به ادمین: {e}")
        await update.callback_query.message.reply_text(
            "⚠️ مشکلی پیش اومد. لطفاً دوباره تلاش کن یا با ادمین تماس بگیر."
        )
        return ConversationHandler.END

    await update.callback_query.message.reply_text(
        "✅ آیتمت با موفقیت ثبت شد و برای ادمین فرستاده شد!\n"
        "نتیجه بررسی بهت اطلاع داده میشه. 🙏"
    )
    ctx.user_data.clear()
    return ConversationHandler.END
...
async def admin_approve(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.send_message(
            chat_id=seller_id,
            text="🎉 آیتمت تایید شد!\nبه زودی در الدرادو و پلتفرم‌های مرتبط لیست میشه.\nممنون از همکاریت 🙏"
        )
    except Exception as e:
        logger.error(f"خطا در ارسال پیام به فروشنده: {e}")
...
async def admin_reject(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.send_message(
            chat_id=seller_id,
            text="❌ متأسفانه آیتم ثبت‌شده‌ات تایید نشد.\n"
                 "اگه سوالی داری با ادمین تماس بگیر.\n"
                 "برای ثبت آیتم جدید /start بزن."
        )
    except Exception as e:
        logger.error(f"خطا در ارسال پیام رد به فروشنده: {e}")
